...
def routing_table(*args, **kwargs) -> typing.Optional[str]:
    try:
        netmiko_connection = args[0]  # Unwrapping the tuple
        result = netmiko_connection.send_command("show ip route", strip_prompt=False, strip_command=False)
        if result:
...
def main(*args, **kwargs):
    return routing_table(*args, **kwargs)
